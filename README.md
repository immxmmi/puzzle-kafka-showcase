...
## 🖥️ Optional: Setup Minikube, Homebrew, Docker, Helm, and kubectl
...
- An **ArgoCD-based showcase** simulating solar panel data flowing through MQTT → Kafka
...
- *(Coming soon)* Cluster using NodePools and KRaft (no Zookeeper):
  ...
  ```
  ⚠️ Currently not available — works only with KRaft mode, not Zookeeper.

All clusters come with **2 Kafka brokers**.

### 3. Kafka Cluster Ready – Connect to It
...
> ⚠️ This only works if you're running **Minikube**. For other Kubernetes clusters, you’ll need to log in manually using the proper `argocd login` command.
